...
class ptzControl(object):
    def __init__(self):
        ...
        self.image_dest = '/home/' + str(self.user) +'/forestfire/forestfire/12-02-2022-12-05/'
        if not os.path.exists(self.image_dest):
            os.makedirs(self.image_dest)
            print("Making dirextory", self.image_dest)
    ...
    def stop(self):
        ...
        print("self.request:{self.requests}")
    ...
    def move_abspantilt(self, pan, tilt, zoom, velocity):
    ...
    def capture(self):
        self.RTSP_URL = "rtsp://" + USER + ":" + streamPass + "@" + IP + ":554/Streaming/Channels/101/Picture"
        ...
        filename = self.image_dest + "frame_" + timestamp + ".jpg"
    ...
    def store_json(self, x,y,z,v,timestamp):
        filename = self.image_dest + "frame_" + timestamp + ".json"
        # gather data
        data_dict = {}
        data_dict['image'] = self.image_dest + "frame_" + timestamp + ".jpg"
        data_dict['parameters'] = {
            'pan': x, 'tilt':y, 'zoom':z, 'speed':v 
        }
    ...
    def get_all_images(self, x, y, z, v, num_segments):
        for i in range(num_segments):
            print("Pan Tilt Zoom Speed", x, y, z, v)
            self.move_abspantilt(x,y,z,v)
            time.sleep(5)
            timestamp = self.capture()
            self.store_json(x,y,z,v,timestamp)
            x += 2 / (num_segments)
            x = round(x,2)
            if (x > 1 or x < -1): 
                break
...
if __name__ == "__main__":
    ...
    opts, args = getopt.getopt(sys.argv[1:], "hd:n:z:", 
                               ["dest=","num_imgs=", "zoom="])
    for opt, arg in opts:
        if opt =="--dest" or opt =="-d":
            ptzControl.set_dest(arg)
        elif opt =="--num_imgs" or opt =="-n":
            num_segments = int(arg)
        elif opt =="--zoom" or opt =="-z":
            z = float(arg)
        else:
            print(opt,'is not a valid argument')
            sys.exit()
    
    # assign pan/tilt/zoom/speed
    z = 0 # zoom
    y = 1 # tilt
    x = -1.0 # initial value - will be changed
    v = 1 # default value for speed

    # ptzControl.get_all_images(x, y, z, v, num_segments)
    DIR = os.getcwd() + '/hello/'
    # ptzControl.custom_zoom(DIR)\
    ptzControl.get_all_images(x,y,z,v,num_segments)
    # stop ptz camera 
    ptzControl.stop()
